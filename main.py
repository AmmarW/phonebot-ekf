...
def main():
    ...
    try:
        ...
        print(f"Static window empty—using first {N} samples for bias")
    ...
    plt.scatter(states_df['px'].iloc[0], states_df['py'].iloc[0], color='green', label='Start', zorder=5)

    # Mark the end point
    plt.scatter(states_df['px'].iloc[-1], states_df['py'].iloc[-1], color='red', label='End', zorder=5)

    plt.xlabel('px (meters)')
    plt.ylabel('py (meters)')
    plt.title('Estimated 2D Trajectory (px vs py) - U turn path')
    plt.legend()
    plt.grid(True)
...
# import numpy as np
# from data_loader import load_csv, estimate_rate
# from preprocessor import trim_initial_static, estimate_biases, apply_calibration, trim_final_static
# from utils import phone_to_robot, latlon_to_enu
# from ekf_filter import EKF
# import matplotlib.pyplot as plt
...
#     accel.rename(columns={'x':'ax','y':'ay','z':'az'}, inplace=True)
#     gyro .rename(columns={'x':'gx','y':'gy','z':'gz'}, inplace=True)
#     orientation.rename(columns={'roll': 'roll', 'pitch': 'pitch', 'yaw': 'yaw'}, inplace=True)  # Ensure proper column names
...
#     gps   = gps  [gps.index   >= t0]
#     print(f"Trimmed start @ {t0} → accel {accel.shape}, gyro {gyro.shape}, gps {gps.shape}")
#     accel = accel[accel.index <= t1]
#     gyro  = gyro [gyro.index  <= t1]
#     gps   = gps  [gps.index   <= t1]
#     print(f"Trimmed end @ {t1} → accel {accel.shape}, gyro {gyro.shape}, gps {gps.shape}")
...
#         psi = pitch +1  # psi is directly the pitch in this case

#         # Predict the state using IMU data and pitch as psi
#         ekf.predict(np.array([ax, ay]), gz, psi)  # Pass pitch as psi
...
#     plt.xlabel('px (meters)')
#     plt.ylabel('py (meters)')
#     plt.title('Estimated 2D Trajectory (px vs py) - Straight Line path 1')
#     plt.legend()
#     plt.grid(True)
#     plt.axis('equal')  # So x and y are scaled equally
#     plt.show()
